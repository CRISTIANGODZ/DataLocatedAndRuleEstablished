<template>
  <el-container direction="vertical">
    ...
    <el-row>
      ...
      <el-table :data="arrList" border highlight-current-row style="width: 100%">
        <el-table-column prop="patientId" width="83" label="病人编号"> </el-table-column>
        ...
        <el-table-column label="操作" width="250">
          <template slot-scope="scope">
            ...
            <el-button size="mini" type="danger" @click="PatientDel(scope)">删除</el-button>
          </template>
        </el-table-column>
      </el-table>
    ...
    <el-dialog title="数据集详情" :visible.sync="checkDatasetModalVisible">
      ...
      <el-row>
        <el-table :data="jsonData.textlist" border highlight-current-row style="width: 100%">
          <el-table-column prop="index" width="100" label="编号"> </el-table-column>
          ...
          <el-table-column prop="title" label="处理操作" width="300">
            <template slot-scope="scope">
              <el-button size="mini" type="danger" @click="PatientDel(scope)">删除</el-button>
            </template>
          </el-table-column>
...
export default Vue.extend({
  ...
  methods: {
    PatientDel(row) {
      let data = this.jsonData.textlist.splice(row.$index, 1)// 删除后的行
      console.log(this.jsonData);
      console.log(row);

      console.log(row.$index); //删除行的索引
      console.log(this.filterText.datasetId); // 数据集id、
      this.$confirm("确定要删除这个数据吗？", "警告", {
        ...
        .then(() => {
          ...
          //   themeId: row.row.themeId,
          //   textId: row.row.textId,
          // }
          // this.$http.post('/electronic/case/patient/text/delete', this.patientData).then((response)=>{
    ...
    getPatients(){
      this.nameVo = this.name;
      this.$http
      .post('/electronic/case/patient/text/get', this.nameVo)
      .then((response)=>{
        if (response.data.code === 200) {
            this.jsonData.datasets = response.patientThemeTextList;
          }
      })
    },
    getDataSets() {
      ...
      this.$http
        ...
        .then((response) => {
          if (response.data.code === 200) {
            ...
            console.log(this.jsonData.datasets);
          }
        })
        .catch((error) => {
          console.log(error);
